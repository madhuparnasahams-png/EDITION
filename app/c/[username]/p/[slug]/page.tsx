      ...
      <article className="max-w-3xl mx-auto px-4 py-12">
        ...
        <header className="mb-12">
          ...
          {isEditing ? (
            ...
            <>
              ...
              {article.description && (
                <p className={`text-xl mb-6 ${darkMode ? "text-gray-400" : "text-gray-600"}`}>{article.description}</p>
              )}
            </>
          )}
          ...
          <div className={`flex items-center gap-4 border-t border-b py-4 ${darkMode ? "border-gray-700" : "border-gray-300"}`}>
            ...
            <div>
              ...
              <p className={darkMode ? "text-sm text-gray-500" : "text-sm text-gray-500"}>
                {new Date(article.publishedAt).toLocaleDateString()}
              </p>
            </div>
            {article.isFree && <span className={`ml-auto text-xs border px-2 py-1 ${darkMode ? "border-gray-600" : "border-gray-400"}`}>FREE</span>}
          </div>
        ...
        <div style={{ lineHeight }} className="text-base select-none">
          {article.blocks && article.blocks.length > 0 ? (
            ...
            <p className={darkMode ? "text-center text-gray-500" : "text-center text-gray-600"}>No content</p>
          )}
        </div>
        ...
        <footer className={`mt-16 border-t pt-8 text-center ${darkMode ? "border-gray-700" : "border-gray-300"}`}>
          <p className={`text-sm mb-4 ${darkMode ? "text-gray-500" : "text-gray-600"}`}>End of article</p>
          ...
          <div className="mb-6">
            {!reposted ? (
              ...
              <span className="text-xs text-gray-500">Reposted to your Comm</span>
            )}
...
function Block({ block, fontSize, darkMode }: { block: Block; fontSize: number; darkMode: boolean }) {
        ...
        {block.content.stickers && block.content.stickers.length > 0 && (
          <div className="absolute bottom-2 right-2 flex gap-1">
            {block.content.stickers.map((url: string) => (
              <img key={url} src={url} alt="" className="w-12 h-12 object-contain pointer-events-none" draggable={false} />
            ))}
          </div>
        )}
        {block.content.alt && (
          <figcaption className={`text-sm text-gray-600 mt-2 ${fullBleed ? 'px-4' : ''}`}>
            {block.content.alt}
          </figcaption>
        )}
      </figure>
    ...
    return (
      <div>
        <iframe
          ...
          className={darkMode ? 'border border-gray-700' : 'border border-gray-300'}
        />
        {block.content.caption && (
          <p className={darkMode ? 'text-sm text-gray-400 mt-2' : 'text-sm text-gray-600 mt-2'}>{block.content.caption}</p>
        )}
      </div>
    );
  }
  ...
  if (block.type === 'quote') {
    return (
      <blockquote className={`border-l-4 pl-4 italic ${darkMode ? 'border-white text-gray-300' : 'border-black text-gray-700'}`}>
        "{block.content.text}"
